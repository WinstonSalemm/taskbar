...
export const getDeadlineStatus = (task) => {
  const now = new Date();
  const actualDeadline = task.actualDeadline
    ? new Date(task.actualDeadline)
    : null;
  const requestedDeadline = task.requestedDeadline
    ? new Date(task.requestedDeadline)
    : null;
    ...
    if (actualDeadline && completedAt > actualDeadline) {
      return {
        ...
        daysOverdue: Math.ceil(
          (completedAt - actualDeadline) / (1000 * 60 * 60 * 24),
        ),
      };
  ...
  if (task.status !== "done" && actualDeadline) {
    if (now > actualDeadline) {
      const daysOverdue = Math.ceil(
        (now - actualDeadline) / (1000 * 60 * 60 * 24),
      );
      return {
        status: "overdue",
      ...
      const daysLeft = Math.ceil(
        (actualDeadline - now) / (1000 * 60 * 60 * 24),
      );
      return {
        status: "on_time",
  ...
  if (task.status !== "done" && requestedDeadline) {
    const daysLeft = Math.ceil(
      (requestedDeadline - now) / (1000 * 60 * 60 * 24),
    );
    if (daysLeft < 0) {
      return {
        status: "overdue",
      ...
      return {
        ...
        label:
          daysLeft === 0
            ? "Желаемый срок сегодня"
            : `Желаемый: ${daysLeft} дн.`,
...
export const sortTasksByPriority = (tasks) => {
  ...
  return tasks.sort((a, b) => {
    ...
    if (
      aDeadlineStatus?.status === "overdue" &&
      bDeadlineStatus?.status !== "overdue"
    )
      return -1;
    if (
      bDeadlineStatus?.status === "overdue" &&
